defmodule LLMModels do
  ...
  alias LLMModels.{Engine, Model, Packaged, Provider, RuntimeOverrides, Spec, Store}
  ...
  `mix llm_models.pull`. It does NOT run the full ETL pipeline or fetch
  from remote sources.
  ...
  - `{:error, :no_snapshot}` - No pre-built snapshot available (run `mix llm_models.pull`)
  ...
  defp apply_runtime_overrides(snapshot, opts) do
    case Keyword.get(opts, :runtime_overrides) do
      nil -> {:ok, snapshot}
      overrides -> RuntimeOverrides.apply(snapshot, overrides)
    end
  end
